...
public:
  ...
  virtual ValueConfiguration* handleValueConfiguration(const std::string& cmd, ValueConfHeader& header) override
  {
    ...
    if (const boost::python::override f = this->get_override("handleValueConfiguration"))
    {
      ...
      {
        ValueConfiguration* result = boost::python::extract<ValueConfiguration*>(res); // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
        ValueConfiguration* newVc  = new ValueConfiguration(*result);                  // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
        return newVc;
      }
      return nullptr;
    }
