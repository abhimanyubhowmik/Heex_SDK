...
ValueConfiguration* RecorderWrap::handleValueConfiguration(const std::string& cmd, ValueConfHeader& header)
{
  ...
  if (const boost::python::override f = this->get_override("handleValueConfiguration"))
  {
    ...
    {
      ValueConfiguration* resultat = boost::python::extract<ValueConfiguration*>(res); // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
      ValueConfiguration* newVc    = new ValueConfiguration(*resultat);                // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
