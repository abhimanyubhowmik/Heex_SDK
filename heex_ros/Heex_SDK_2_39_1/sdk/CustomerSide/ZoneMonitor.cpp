...
ValueConfiguration* ZoneMonitor::handleValueConfiguration(const std::string& cmd, ValueConfHeader& header)
{
  ...
  {
    ValueConfigurationZone* vcz = new ValueConfigurationZone(header); // NOLINT: not using smart pointer for _valueConfigurations due to backwards compatibility
    const bool isVcValid        = vcz->isValid();
    if (!isVcValid)
    {
      HEEX_LOG(error) << "ZoneMonitor::handleValueConfiguration() Invalid input command : '" << cmd << "'" << std::endl;
      delete vcz; // NOLINT: not using smart pointer for _valueConfigurations due to backwards compatibility
      return nullptr;
    }
