...
ValueConfiguration* IntervalMonitor::handleValueConfiguration(const std::string& cmd, ValueConfHeader& header)
{
  ...
  {
    ValueConfigurationDoubleInterval* vcdi = new ValueConfigurationDoubleInterval(header); // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
    ...
    if (!isVcValid)
    {
      ...
      delete vcdi; // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
      return nullptr;
    }
  ...
  {
    ValueConfigurationThreshold* vcthr = new ValueConfigurationThreshold(header); // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
    const bool isVcValid               = vcthr->isValid();
    if (!isVcValid)
    {
      HEEX_LOG(error) << "IntervalMonitor::handleValueConfiguration() Invalid input command : '" << cmd << "'" << std::endl;
      delete vcthr; // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
      return nullptr;
    }
