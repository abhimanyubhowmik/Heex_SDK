...
ValueConfiguration* StringMonitor::handleValueConfiguration(const std::string& cmd, ValueConfHeader& header)
{
  ...
  {
    ValueConfigurationString* vccb = new ValueConfigurationString(header); // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
    ...
    if (!isVcValid)
    {
      ...
      delete vccb; // NOLINT: not using smart pointer for valueConfigurations due to backwards compatibility
      return nullptr;
    }
