...
int main(int argc, char** argv)
{
  ...
  const std::string monitorUuid  = vm["monitor_uuid"].as<std::string>();
  const std::string recorderUuid = vm["recorder_uuid"].as<std::string>();
  ...
  //
  MonitorADTransitionDisengagement monitorBagDisengagement(monitorUuid, "127.0.0.1", 4242);
  RecorderRosbagDataLakeExtractor recorderBagExtractor(recorderUuid, "127.0.0.1", 4243, inputBag, recorderOuputPath, recorderPositionTopic, topics_tf);
  ...
  BOOST_FOREACH (const rosbag::MessageInstance m, viewQuery)
  {
